...
for trial=1:length(sigmaProps)
  sigma_prop = sigmaProps(trial); % Standard deviation of the Gaussian proposal.
...
%seed = 1; randn('state', seed); rand('state', seed);
%xinit = 20*rand(1,1); % initial state
%[x2, naccept] = metrop(@target, @proposal, xinit, Nsamples,  targetArgs, proposalArgs, @proposalProb);
...
figure;
plot(x)
title(sprintf('%s=%5.3f', '\sigma', sigma_prop))
