...
function p = generalizationUninfPrior(y,X)
    ...
    %p = 1 ./ ((n*(n-1)*n*(n-1))*((X1scale + d1).^(n-1)).* ((X2scale + d2).^(n-1)));
...
%%
% Calculate d1,d2 from the data. dj(i) is 0 whenever y(i,j) is within the
% span of the data X, otherwise its the distance to the nearest neighbour
% along the jth dimension.
function [d1 d2 X1scale X2scale] = neighbour(y,X)
    [X1min X1max X2min X2max X1scale X2scale] = calcDataRange(X,size(X,1));   
    d1 = ((y(:,1) < X1min).*abs(y(:,1) - X1min)) + ((y(:,1) > X1max).*abs(X1max - y(:,1)));
    d2 = ((y(:,2) < X2min).*abs(y(:,2) - X2min)) + ((y(:,2) > X2max).*abs(X2max - y(:,2)));    
end
...
function plotPriorPost
    figure;
    plotSurface(@(h)priorFn(h),[],no,no,no,[0 1 0 1 0 0.03],priorType);
    shading interp;
    if doPrint
    ...
    for i=1:length(points)
      ...
      scale = [];
      plotSurface(@(h,data)posteriorFn(h,data),data(1:points(i),:),yes,no,no,...
      		scale,...
      		{sprintf('Posterior, n=%d, %s', points(i), priorType)});
      shading interp;
      
      if doPrint
      fname = sprintf('%s/healthyLevelsPost%d%s.eps', folder, points(i), priorType)
...
for i=1:length(points)
    figure;
    ttl = sprintf('samples from p(h|D(1:%d)), %s', points(i), priorType);
    %plotSampledHypotheses(@(h,data)posteriorFn(h,data(1:points(i),:)),...
...
function plotSampledHypotheses(post,data,newFigure,plotTitle)
    ...
    hold on
    [cMin cMax iMin iMax cScale iScale] = calcDataRange(data,size(data,1));
    ...
    for i=1:N
        rectangle('Position',[L(i,1), L(i,2), sampleScale1(i), sampleScale2(i)],'LineWidth',3,...
            'EdgeColor',colors(i,:)); 
    end
    title(plotTitle); 
...
%%
%Plot the data and the smallest rectangle completely enclosing that data
...
function plotSurface(func,data,dependsOnData,solid,newFigure,scale,plotTitle)
    if(newFigure)
        figure;
    end
