...
%%
%  models = ModelSelection.formatModels(0:0.1:1, 1:10, 0:1, 1:3)
...
msCVmse = ModelSelection(           ...
    'testFunction',testFunction     ,...      % the test function we just created
    'Xdata'       ,Xtrain           ,...      % all of the X data we have available
    'Ydata'       ,ytrain           ,...      % all of the y data we have available
    'verbose'     ,false            ,...      % turn off progress report
...
% function. Recall that the output of our testFunction is passed directly to the
% loss function. In this case we will have the testFunction simply return the
