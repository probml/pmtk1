classdef SampleDist < ParamFreeDist
  ...
  properties
    samples; % rows are samples, columns are dimensions
             ...
             % samples(s,d,i)
    weights; % the (proportional) probability mass for each sample
             % this will become useful when we implement smc methods
    domain; 
  ...
  %%  Main methods
  methods
    function m = SampleDist(X, domain, support, varargin)
      [weights] = process_options(varargin, 'weights', []);
      ...
      m.support = support;
      m.weights = normalize(ones(size(X,1), 1));
    end
    ...
    % in parallel). mu is of size npdfs-by-d. 
      mu = squeeze(mean(bsxfun(@times, obj.weights, obj.samples)))';
    end
    ...
    % v is of size npdfs-by-d    
      v = squeeze(var(bsxfun(@times, obj.weights, obj.samples)))';
    end
    ...
    % C is of size d-by-d-npdfs
      ...
      C = zeros(d,d,npdfs);
      for i=1:npdfs
         C(:,:,i) = cov(bsxfun(@times, obj.weights, obj.samples));
      end
    end
    ...
    % mm is of size nsamples-by-numel(queryVars)-by-npdfs  
      ...
      mm = SampleDist(m.samples(:,Q,:), m.weights, m.domain(Q));
    end
...
%% Working line by line through the file, this is where you have stopped modifying SampleDist for SMC support
