...
% OutputFn - if nonempty, called at each iteration thus
%     stop = outputFn(x, optimValues, state);
...
% ftrace - objective function value at each iteration
%
...
[verbose,verboseI,debug,doPlot,maxFunEvals,maxIter,tolFun,tolX,method,...
    ...
    DerivativeCheck,Damped,HvFunc,bbType,cycle,outputFn] = minFunc_processInputOptions(options);
...
for i = 1:maxIter
    ...
    %%% KPM - record history of function values
    ftrace(i+1) = f;
    
    if ~isempty(outputFn) %%% KPM
      optimValues.iteration = i;
      optimValues.fval = f;
      optimValues.funcount = funEvals*funEvalMultiplier;
      state = 'iter';
      stop = outputFn(x, optimValues, state);
    end
