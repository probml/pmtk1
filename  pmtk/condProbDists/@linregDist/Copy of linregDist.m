  ...
  methods 
    function m = linregDist(varargin)
     ...
     function p = logprob(obj, X, y)
      ...
      function [muHat, py] = predict(obj, X)
      ...
      function obj = inferParams(obj, varargin)
        ...
        [X, y, lambda, prior] = process_options(...
     ...
     function s = bicScore(obj, X, y)
       L = sum(logprob(obj, X, y));
       n = size(X,1);
       %d = length(obj.w);
       d = dofRidge(obj, X, obj.lambda);
       s = L-0.5*d*log(n);
     end
     
     function s = aicScore(obj, X, y)
       L = sum(logprob(obj, X, y));
       n = size(X,1);
       %d = length(obj.w);
       d = dofRidge(obj, X, obj.lambda);
  ...
  methods(Static = true)
    ...
    function helperPolyFitN(deg)
      ...
      for i=1:length(ns)
        ...
        m = linregDist('paramEstMethod', 'ridgeQR');
        m.transformer =  chainTransformer({rescaleTransformer, polyBasisTransformer(deg)});
        m = fit(m, 'X', xtrain, 'y', ytrain, 'lambda', lambda);
    ...
    function demoPolyFitRidgeBIC(n)
      ...
      for i=1:length(lambdas)
        ...
        bic(i) = bicScore(m, xtrain, ytrain);
        aic(i) = aicScore(m, xtrain, ytrain);
        nparams(i) = length(m.w);
      end
      figure;
      hold on
    ...
    function helperBasis(varargin)
      ...
      switch prior
        case 'mvn'
          m = linregDist('transformer', T, 'w',  'vagueMVNprior', ...
            'sigma2', sigma2);
        case 'mvnIG'
          m = linregDist('transformer', T, 'w', 'vagueMVNIGprior');
      end
      
      m = inferParams(m, 'X', xtrain, 'y', ytrain);
    ...
    function hh=helperGaussVsNIG(varargin)
      ...
      switch prior
        case 'mvn'
          m = linregDist('transformer', T, 'w', 'vagueMVNprior', 'sigma2', sigma2);
        case 'mvnIG'
          m = linregDist('transformer', T, 'w', 'vagueMVNIGprior');
      end
      m = inferParams(m, 'X', xtrain, 'y', ytrain);
     ...
     function hh=demoPolyFitErrorBars(varargin)
      ...
      for i=1:length(degs)
        ...
        switch prior
          case 'mvn'
            m = linregDist('transformer', T, 'w', 'vagueMVNprior', 'X', xtrain, 'sigma2', sigma2);
          case 'mvnIG'
            m = linregDist('transformer', T, 'w', 'vagueMVNIGprior', 'X', xtrain);
        end
        m = inferParams(m, 'X', xtrain, 'y', ytrain);
        ypredTest = postPredict(m, xtest);
        figure;
